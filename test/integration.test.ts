...
describe('integration tests (requires local docker compose)', () => {
  ...
  test('connect REST is reachable', async () => {
    ...
    const res = await axios.get(`${CONNECT_URL}/` , { timeout: 5000 });
    expect(res.status).toBe(200);
  }, 20000);

  test('list connectors', async () => {
    if (process.env.INTEGRATION !== '1') return;
    const res = await axios.get(`${CONNECT_URL}/connectors`, { timeout: 5000 });
    expect(res.status).toBe(200);
  ...
  test('list subjects from both schema registries', async () => {
    ...
    const r2 = await axios.get(`${SR2}/subjects`, { timeout: 5000 });
  ...
  test('copy a subject schema from SR1 to SR2 if any subject exists', async () => {
    ...
    const versions = await axios.get(`${SR1}/subjects/${encodeURIComponent(subject)}/versions`, { timeout: 5000 });
    const version = versions.data[0];
    const schema = await axios.get(`${SR1}/subjects/${encodeURIComponent(subject)}/versions/${version}`, { timeout: 5000 });
